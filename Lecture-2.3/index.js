...
var Handler = {
    "LaunchRequest" : function()
	{
		...
		{
			this.emit(':askWithCard',"For a better experience,please link your account","For a better experience,please link your account","Account Linking","For a better experience,please link your account");
		}else
		{
            this.emit(":askWithCard","Hi , you can ask me the current bitcoin arbitrage","Hi,you can get information about the current bitcoin arbitrage or you can set an mail alert","Bitcoin  Skill","Hi ,say get info or mail me");
		}
   },
    "SpotPriceIntent" : function(){
	    ...
	    cheerioReq("http://cfe.cboe.com/cfe-products/xbt-cboe-bitcoin-futures", (err, $) => {         //Used for obtaining the present Bitcoin futures value in cboegroup and its current date
		 ...
		 CboeDate = $("#itemPlaceholderContainer1 > tr:nth-of-type(3) > td:nth-of-type(2)").text();
			...
			this.emit(':ask',"The Bitcoin Price is " + Price + " dollars,The " 
			+ CmeDate + " Bitcoin Futures contract on the CME is trading at " + CmeValue + " dollars,This is " 
			+ CmeDiff + " dollars " + CmeSign + " the Spot Bitcoin price,The " + CboeDate + 
			" expiring Bitcoin futures contract on the C.B.O.E is trading at " + CboeValue + " dollars,This is " 
			+ CboeDiff + " dollars " + CboeSign + " the Spot Bitcoin price,The "
			+Cmemonth+" Bitcoin Futures contract on the CME,is trading at " + GranDiff+ " dollars " 
			+ GranSign + " the C.B.O.E Bitcoin Futures contract,Would you like to set a mail alert");    //Specifies the speech output to be spoken by alexa
		  },
	...
	"SetSpreadIntent"  : function(){
		 ...
		 var params =                   //The user's Email ID is sent along with his set spread value and UserID to be stored in dynamoDB
		{
		  ...
		  ddb.putItem(params, function(err, data) {  //Stores the data in dynamoDB
		  if (err) {
			...
			if(Email == undefined){
				 context.emit(':ask',"Please try again");   //If Email is not parsed correctly,prompts the user to retry
			}
			context.emit(':tellWithCard',"A mail will be sent to your amazon account "+Email+
			" when the arbitrage crosses "+Spread+" dollars","Mail Alert","A mail will be sent to your amazon account "
			+Email+" when the arbitrage crosses "+Spread+" dollars");
	...
	"AMAZON.HelpIntent" : function(){
			this.emit(':tell', "For querying about the current bitcoin arbitrage ,ask What is the current bitcoin arbitrage , For setting a mail alert say mail me and specify the required value in dollars");
		},
	"Unhandled" : function(){
			this.emit(':ask', "Sorry,I didn't get that,For querying about the current bitcoin arbitrage ,ask What is the current bitcoin arbitrage , For setting a mail alert say mail me and specify the required value in dollars");
		}
	};